...
def generate_menu_items(restaurants_df):
    ...
    menu_items_data = []
    current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    restaurants_records = restaurants_df.to_dict('records')
    ...
    for restaurant in restaurants_records:
        ...
        for item in master_menu:
            ...
            menu_items_data.append({
                ...
                "is_vegetarian": int(item["is_vegetarian"]),  # BIT expects 0/1, not True/False
...
def generate_data_for_sql_db():
    ...
    df_menu_items = generate_menu_items(df_restaurants)
    df_customers = generate_customers(500)

    dataset_dir = os.path.join(script_dir, "..", "3_datasets")
    os.makedirs(dataset_dir, exist_ok=True)

    df_restaurants.to_csv(os.path.join(dataset_dir, "restaurants.csv"), index=False)
    df_menu_items.to_csv(os.path.join(dataset_dir, "menu_items.csv"), index=False)

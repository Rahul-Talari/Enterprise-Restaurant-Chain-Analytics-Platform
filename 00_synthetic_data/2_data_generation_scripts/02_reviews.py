...
df_orders = pd.read_csv(os.path.join(dataset_dir, "historical_orders.csv"))
...
def generate_customer_reviews(review_percentage=0.35):
    ...
    reviews = []
    generation_time = datetime.now().strftime(TIMESTAMP_FORMAT)
    ...
    for idx, order in df_orders.iterrows():
        # Only a subset of orders get reviews
        ...
        order_date = datetime.strptime(order['order_timestamp'], TIMESTAMP_FORMAT)
        ...
        review = {
            ...
            "rating": rating,
            "review_timestamp": review_ts.strftime(TIMESTAMP_FORMAT),
    ...
    df_reviews.to_csv(os.path.join(dataset_dir, "customer_reviews.csv"), index=False)
...
if __name__ == "__main__":
    generate_customer_reviews(review_percentage=0.35)